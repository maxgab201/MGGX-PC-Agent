...
- Agent token: independent 256-bit base64url secret copied from MGGX PC Agent Control.
...
```json
{"ok":true,"service":"mggx-pc-agent","apiVersion":1,"agentVersion":"1.0.0","uptimeSeconds":1234}
```
...
All routes are `POST`, require the Bearer token, and have an empty request body.

| Route | Success | State | Unsupported/missing |
|---|---:|---|---|
| `/api/v1/power/shutdown` | 202 | `shutting_down` | — |
| `/api/v1/power/restart` | 202 | `restarting` | — |
| `/api/v1/power/sleep` | 202 | `sleeping` | 409 `sleep_not_available` |
| `/api/v1/power/hibernate` | 202 | `hibernating` | 409 `hibernate_not_available` |
| `/api/v1/power/lock` | 202 | `locking` | — |
| `/api/v1/services/sunshine/restart` | 202 | `restarting` | 409 `sunshine_not_installed` |

Accepted response: `{"ok":true,"state":"shutting_down"}`. Error response: `{"ok":false,"error":"hibernate_not_available"}`.
...
After sending WOL, poll `/health` every 1–2 seconds with a practical overall deadline (for example 120 seconds). Transition `WAKING → ONLINE` only after health succeeds. Then fetch authenticated status. After shutdown returns 202, poll health until it stops responding and transition to `OFFLINE`.
