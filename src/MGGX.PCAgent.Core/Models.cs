...
public sealed record AgentStatus(bool Ok, int ApiVersion, string AgentVersion, PcStatus Pc, WindowsStatus Windows,
    ComponentStatus Sunshine, ComponentStatus Tailscale, PowerCapabilities Power, string? LanIp = null);
...
public sealed class AgentConfig
{
    ...
    public string? LanAdapterId { get; set; }
    public string PcId { get; set; } = "main";
    public string PcName { get; set; } = "MGGX PC";
    ...
    public bool IsValid() => Port is > 0 and <= 65535 && DiscoveryPort is > 0 and <= 65535 &&
        ...
        !string.IsNullOrWhiteSpace(PcId) && !string.IsNullOrWhiteSpace(PcName);
}
